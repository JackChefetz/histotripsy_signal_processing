...
% HERE
% External Process 5 for Dissolution Time
...
% HERE
Event(n).info = 'Dissolution Time'; % Event to process UF data
Event(n).tx = 0; % no TX structure.
Event(n).rcv = 0; % no Rcv structure.
Event(n).recon = 0; % no reconstruction.
Event(n).process = 5; % run 'speedy_processing'
Event(n).seqControl = 0;
n=n+1;
...
%pcidata = false;
ufdata  = false;
UI(7).Control = {'UserC3', 'Style', 'VsPushButton', 'Label', 'Dissolution Time'};
