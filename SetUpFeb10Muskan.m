...
Event(n).info = 'test'; % Event to save Bmode
Event(n).tx = 0; % no TX structure.
Event(n).rcv = 0; % no Rcv structure.
Event(n).recon = 0; % no reconstruction.
...
% 
test = false;
UI(7).Control = {'UserC3', 'Style', 'VsToggleButton', 'Label', 'testing'};
UI(7).Callback = {'assignin(''base'',''test'',UIState);'};
...
%%% NOTE: IF I RUN THE COMMENTED SECTION, THE CODE RUNS FINE.%%%
% global t_count
% nskip   = evalin('base', 'nskip');
% if test
%     if isempty(t_count)
...
%%% NOTE: WHEN I TRY TO RUN THIS, I GET ERRORS %%%
if test
    Trans = evalin('base', 'Trans');
    speedyprocessingEDIT(RData, Trans);
else 
    assignin('base','test', false);
    return
end
