...
function ttp=speedy_processing_KBB_2025March5(RData, PData, Trans, TW, P, Receive3, PRF);

% initialize dictonary to store results
persistent ttp_dict ttp_mat loop mGS
if isempty(ttp_dict)
    ttp_dict = {};  % A cell array to store {time, ttp} pairs
end 

if isempty(ttp_mat)
    ttp_mat = [];  % A cell array to store {ttp} pairs
end 

tw1 = TW(1);
tw1 = tw1.Waveform;
% Modified lines 213 to 217 in SetUp file. See if this simplifies
...
tw2 = TW(4).Waveform; %tw2.TW.Waveform; % transmit waveform is in the TW structure 
...
elseif loop == 5
    ...
    % KBB: This is good, but it needs to be saved somewhere. See suggestion
    % below
    ...
    save(dfn, 'ttp_dict', 'ttp_mat','-append')

    % Now, update PRF

    tmp = ttp_mat;

    if isempty(tmp) || isnan(nanmean(tmp(:)))
        tTP = PRF;    % IF there is not data, just use initial PRF
    else  
        tTP = nanmean(tmp(:)); % Here, do some temporal averaging of 5 datapoints. This is arbitrary at this point. if not enough data, just average whole dataet
        tTP = round(1e3/tTP);
    %     else
    %         tTP = nanmean(tmp((end-5):end)); % Temporal average
    %         tTP = round(1e3/tTP);
    end
