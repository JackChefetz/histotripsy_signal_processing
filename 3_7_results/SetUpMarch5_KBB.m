...
addpath('C:\Users\verasonics\Documents\Vantage-4.5.3-2107301223\PME - UF seq img')

% Where data is stored
dfn = sprintf('C:\\Users\\verasonics\\Documents\\Vantage-4.5.3-2107301223\\PME - UF seq img\\data.mat');
%ttp_dict = [];
ttp_mat  = [];
save(dfn, 'ttp_mat', '-append')
...
% Load Chirp
tt = load('C:\Users\verasonics\Documents\Vantage-4.5.3-2107301223\PME - UF seq img\Setup Data\SH_Chirp_2024March22.mat', 'TW');
%tt = load('Chirp_2024March22.mat', 'TW');
tt.TW.Parameters = [];
TW(4) = tt.TW;
...
SeqControl(3).argument = 250;  % 1 msec
...
nsc = 8; %length(SeqControl)+1; % nsc is count of SeqControl objects
...
for i = 1:Resource.RcvBuffer(2).numFrames                     % Acquire frame
     ...
     Event(n).recon = 0;
     Event(n).process = 0;
     Event(n).seqControl = [5, 7, nsc, 3];  %[5, nsc, 3];
...
Event(n).info = 'DissolutionTime'; % Event to save Bmode
Event(n).tx = 0; % no TX structure.
Event(n).rcv = 0; % no Rcv structure.
Event(n).recon = 0; % no reconstruction.
Event(n).process = 5; % run 'ProcBmode_no_US.m'
Event(n).seqControl = 0;
n=n+1;
...
% 
DissolutionTime = false;
UI(7).Control = {'UserC3', 'Style', 'VsToggleButton', 'Label', 'DissolutionTime'};
UI(7).Callback = {'assignin(''base'',''DissolutionTime'',UIState);'};
...
%% Start histotripsy pulse
addpath('C:\Users\verasonics\Documents\MATLAB\FPGA\FPGA_MATLAB\Current_Driver_Functions')
VOUT = 250; %Voltage Applied to the transducer
PRF  = 10; %[Hz]   Initial PRF

% Set voltage to transducer
setVOUT(VOUT)

% Run FPGA to set initial parameters
FPGA_Renal_Cobot_Fnc(PRF);
FPGA_Renal_Cobot_Fnc(PRF);
...
if DissolutionTime
    ...
    PRF = evalin('base', 'PRF');
    %tw1 = evalin('base', 'TW(1)');
    %tw2 = evalin('base', 'TW(2)');
    P = evalin('base', 'P');
    TW = evalin('base', 'TW');
    Receive3 = evalin('base', 'Receive3');
    speedy_processing_KBB_2025March5(RData, PData, Trans, TW, P, Receive3, PRF);
else 
